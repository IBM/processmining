(function () {
    ...
    function filterAndComputeMetrics_fake(trace) {
    ...
    function filterAndComputeMetrics(trace) {
        ...
        for (var k = 0; k < trace.size(); k++) {
            ...
            if (event.getEventClass() == this.ACTIVITY) { // don't break if found, we need parse all the events for the leadtime
    ...
    function computeScale(groupBy, aDate) {
        ...
        if (this.times.length == 0) { // first event
            ...
            for (k = 0; k < this.dataset.length; k++) {
                ...
                this.dataset[k]['leadtime_sums'] = appendCounters.concat(this.dataset[k]['leadtime_sums']);
            }
        }
            ...
            while (aDate.getTime() < eventTime) {
                ...
                for (k = 0; k < dataset.length; k++) {
                    ...
                    this.dataset[k]['leadtime_sums'].push(0); // add 0 for each new date, to each counter array
                }
            }
        }
        return eventTime;
        ...
        update: function (trace) {
            ...
            if (valueindex < 0) {// value not yet added; add it to dataset
                ...
                for (var ii = 0; ii < dataset[0]['counters'].length; ii++) {
                    ...
                    this.dataset[valueindex]['leadtime_sums'].push(0);
                }
            }

            // update the counters
            this.dataset[valueindex]["counters"][this.times.indexOf(eventTime)]++;
            this.dataset[valueindex]["leadtime_sums"][this.times.indexOf(eventTime)] += metrics['leadtime'];
        },
