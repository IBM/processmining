...
function computeScale(GROUPBY, aDate) {
    ...
    if (this_times.length == 0) { // first event
        ...
        values.forEach(element => this_dataset.push({ 'value': element, 'counters': [0] }));
    }
...
function trace(totalevents) {
    for (var i = 0; i < totalevents; i++) {
        ...
        if (this_dataset.length == 0){ // first event
            this_dataset.push({ "value": metrics['value'], "counters": [1] });
            break;
        }
        ...
        if (valueindex < 0) {// value not yet added; add it to this_dataset
            valueindex = this_dataset.push({ "value": metrics['value'], "counters": [] }) - 1;
            // update counters for each groupby date
