...
def init_params_headers(config, headers, params):
    ...
    headers.update({"content-type": "application/json",
                   "Authorization": "Bearer %s" % config['sign']})
...
def ws_delete_process(config):
    ...
    url = "%s/integration/processes/%s" % (
        config['url'], config['project_key'])

    if (PRINT_TRACE):
        print("REST CALL: " + url+" "+str(params))
...
def ws_proc_post(config):
    ...
    if (PRINT_TRACE):
        print("REST CALL: " + url+" "+str(data))
    r = requests.post(url, verify=False, data=json.dumps(
        data), headers=headers, params=params)
    if (r.status_code != 200):
        print("Error: Process %s creation: %s" %
              (config['project_name'], r.json()['data']))
    return r
...
def ws_csv_upload(config):
    ...
    url = "%s/integration/csv/%s/upload" % (
        config['url'], config['project_key'])
    if (PRINT_TRACE):
        print("REST CALL: " + url+" "+str(params))
    files = {'file': (config['csv_filename'], open(
        config['csv_filename'], 'rb'), 'text/zip')}
...
def ws_backup_upload(config):
    ...
    url = "%s/integration/processes/%s/upload-backup" % (
        config['url'], config['project_key'])

    if (PRINT_TRACE):
        print("REST CALL: " + url+" "+str(params))
    files = {'file': (config['backup_filename'], open(
        config['backup_filename'], 'rb'), 'text/zip')}
    r = requests.post(url, verify=False, files=files,
                      params=params, headers=headers)
    if (r.status_code != 200):
...
def ws_get_backup_list(config):
    ...
    url = "%s/integration/processes/%s/backups" % (
        config['url'], config['project_key'])
    if (PRINT_TRACE):
        print("REST CALL: " + url+" "+str(params))
...
def ws_apply_backup(config):
    ...
    url = "%s/integration/processes/%s/backups/%s" % (
        config['url'], config['project_key'], config['backup_id'])
    if (PRINT_TRACE):
        print("REST CALL: " + url+" "+str(params))
    r = requests.put(url, verify=False, headers=headers, params=params)
    if (r.status_code != 200):
        print("Error: apply backup: %s : %s" %
              (r.json()['data'], config['backup_id']))
    return r
...
def ws_create_log(config):
    ...
    url = "%s/integration/csv/%s/create-log" % (
        config['url'], config['project_key'])
    if (PRINT_TRACE):
        print("REST CALL: " + url+" "+str(params))
...
def ws_get_csv_job_status(config):
    ...
    url = "%s/integration/csv/job-status/%s" % (
        config['url'], config['job_key'])
    if (PRINT_TRACE):
        print("REST CALL: " + url+" "+str(params))
    r = requests.get(url, verify=False, headers=headers, params=params)
    if (r.status_code != 200):
...
def ws_get_dashboards(config):
    ...
    url = "%s/analytics/integration/dashboard/%s/list" % (
        config['url'], config['project_key'])
...
def ws_get_widgets(config, dashboard_name):
    ...
    url = "%s/analytics/integration/dashboard/%s/%s/list" % (
        config['url'], config['project_key'], dashboard_id)
...
def ws_create_update_variables(config, variablesArray):
    ...
    url = "%s/integration/processes/%s/variables" % (
        config['url'], config['project_key'])
    r = requests.post(url, verify=False, data=json.dumps(
        variablesArray), params=params, headers=headers)
    return r
...
def ws_get_variable(config, variablename):
    ...
    url = "%s/integration/processes/%s/variables/%s" % (
        config['url'], config['project_key'], variablename)
    r = requests.get(url, verify=False, params=params, headers=headers)
    if (r.status_code == 200):
        values = r.json()
        return values['data']
